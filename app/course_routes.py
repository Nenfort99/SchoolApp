...
@course_router.get('/courses', response_model=List[Course_Pydantic])
async def get_all_courses():
    return await Course_Pydantic.from_queryset(Course.all())

@course_router.post('/course', response_model=Course_Pydantic)
...
@course_router.get('/course/{id}', response_model=Course_Pydantic, responses={404: {"model": HTTPNotFoundError}})
async def get_a_course(id: int):
    return await Course_Pydantic.from_queryset_single(Course.get(id=id))

@course_router.put("/course/{id}", response_model=Course_Pydantic, responses={404: {"model": HTTPNotFoundError}})
async def update_a_course(id: int, course: CourseIn_Pydantic):
    await Course.filter(id=id).update(**course.dict(exclude_unset=True))
    return await Course_Pydantic.from_queryset_single(Course.get(id=id))


@course_router.delete("/course/{id}", response_model=Message, responses={404: {"model": HTTPNotFoundError}})
